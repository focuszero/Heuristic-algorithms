...
int main(int argc, char const *argv[])
{
	...
	vector<int> genoma;
	player random = player(c, s, lado, equipo, equipoRival, genoma);

	vector<vector<int> > ubicacionInicial = random.calcularUbicacionInicialRandom();
	random.darUbicacion(ubicacionInicial);
	...
	for(int j = 0; j < s; j++)
	{
		...
		random.nuevaPosicionPelota(posicionPelota);

		vector<jugador> equipo = {jugadores[0], jugadores[1], jugadores[2]};
		vector<jugador> equipoRival = {jugadores[3], jugadores[4], jugadores[5]};
		random.actualizarJuego(equipo, equipoRival);

		vector<vector<int> > proximoMov = random.calcularProxMovRandom();
		random.darJugada(proximoMov);
	}
